...
if ( ! class_exists( 'Kirki' ) ) {
	...
	if ( class_exists( 'WP_Customize_Section' ) && ! class_exists( 'seopress_Installer_Control' ) ) {
		/**
		 ...
		 */
		class seopress_Installer_Control extends WP_Customize_Control {
			...
			public function render_content() { ?>
				...
				<?php else : ?>
					<hr>
					<p><?php printf( __( 'Kirki plugin is <b>installed but not activated</b>. Please <a href="%s">activate it</a>.', 'seopress' ), admin_url( 'plugins.php' ) ); ?></p>
				<?php endif;
			}
		}
	...
	if ( ! function_exists( 'seopress_installer_register' ) ) {
		/**
		 * Registers the section, setting & control for the kirki installer.
		 */
		function seopress_installer_register( $wp_customize ) {
			// Add the section/
			...
			$wp_customize->add_section( 'kirki_installer', array(
				'title'       => '',
				'description' => esc_attr__( 'If you want to take full advantage of the options this theme has to provide, please install and activate the Kirki plugin.', 'seopress' ),
				'priority'    => -10,
			...
			$wp_customize->add_control( new seopress_Installer_Control( $wp_customize, 'kirki_installer', array(
				'section' => 'kirki_installer',
			) ) );

		}
		add_action( 'customize_register', 'seopress_installer_register' );
	}
}
